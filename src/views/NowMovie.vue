<template>
  <div class="movie-now">
       <div class="movie-now-top">
            ...
            <div class="movie-top-list" v-for="vl in moviesNow" :key="vl.id">
              <div class="movie-now-pho" @click="aaa">
                <img class="auto-img" :src="vl.images.medium" alt />
              </div>
              <div class="movie-now-text">
                <span>{{vl.original_title}}</span>
                <p class="pp">{{getDate(vl.pubdates[0])}}</p>
              </div>
              </div>
            </div>
            </div>
    ...
    <ul class="movies-list">
      <li class="movie-list" v-for="v in movies" :key="v.id">
        ...
        <div class="movie-text">
          ...
          <div class="movie-audience">
            <span class="count">{{v.collect_count}}</span><span class="people">人想看</span>
          </div>
          <div class="movie-star">
            主演：<span>{{getNamesAll(v.casts)}}</span>
          </div>
          <div class="movie-star"><span>{{getTimes(v.pubdates[0])}}</span><span>上映</span>
          </div>
        </div>
        <div
          class="movie-buy"
